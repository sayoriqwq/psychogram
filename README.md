...
## Prelude Artifact release gate

Artifact 使用精确的 `pnpm@10.33.2`。在
`@sayoriqwq/prelude-contract@0.1.0` 发布到 registry 前，本仓库不提交含
`file:/tmp/...` 的伪造 lockfile；Contract 发布后，必须生成并提交正常的
`pnpm-lock.yaml`，再发布 Artifact。已安装依赖的干净源码树可用
`pnpm verify:clean` 验证 build、typecheck、test、assets 和 pack。
